...
class Painel1:
    ...
    def autenticar(self):
        ...
        while tentativas < max_tentativas:
            os.system('clear')
            self.banner()
            print(f"{self.C_WHITE}┌──({self.C_RED}kali🔒vbought{self.C_WHITE})-[{self.C_KALI}login{self.C_WHITE}]{self.RESET}")
    ...
    def banner(self):
        ...
        print(f"{self.C_KALI}")
        print(r"  ____  _____     _    ____  ___  _  _ ")
        print(r" |  _ \|  _  |   / \  / ___|/ _ \| \| |")
        print(r" | | | | |_) |  / _ \| |  _| | | | .` |")
        print(r" | |_| |  _ <  / ___ \ |_| | |_| | |\  |")
        print(r" |____/|_| \_\/_/   \_\____|\___/|_| \_|")
        print(f"{self.C_GRAY}====================================================={self.RESET}")
        print(f" {self.C_WHITE}[+] DRAGON FRAMEWORK v2.1{self.RESET} | {self.C_GRAY}Dev:{self.RESET} {self.C_GREEN} vbought {self.RESET}")
    ...
    def abrir_interface(self):
        ...
        while True:
            ...
            print(f"{self.C_WHITE}┌──({self.C_KALI}kali㉿vbought-termux{self.C_WHITE})-[{self.C_GRAY}~/painel{self.C_WHITE}]{self.RESET}")
            print(f"{self.C_WHITE}└─► Opções disponíveis:{self.RESET}\n")
            
            print(f"  {self.C_KALI}[1]{self.RESET} Port Scanner (TCP Nativo)")
            print(f"  {self.C_KALI}[2]{self.RESET} Resolver IP / DNS")
            print(f"  {self.C_KALI}[3]{self.RESET} Web Banner Grabbing (HTTP)")
            print(f"  {self.C_KALI}[4]{self.RESET} Dispositivos na Rede Local (LAN Scan)")
            print(f"  {self.C_KALI}[5]{self.RESET} Limpar Console")
            print(f"  {self.C_RED}[0]{self.RESET} Encerrar Painel")
            print("")

            try:
                opcao = input(f"{self.C_WHITE}┌──({self.C_KALI}kali㉿vbought{self.C_WHITE})-[{self.C_RED}menu{self.C_WHITE}]\n└─{self.C_GREEN}$ {self.RESET}").strip()
            except KeyboardInterrupt:
                print(f"\n\n{self.C_RED}[!] Encerrando... Até mais, vbought!{self.RESET}")
                break
